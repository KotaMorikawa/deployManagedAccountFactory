...
import "../external-deps/EIP712.sol";
import "../external-deps/EnumerableSet.sol";
...
library AccountPermissionsStorage {
    ...
    function accountPermissionsStorage()
        internal
        pure
        returns (Data storage accountPermissionsData)
    {
...
abstract contract AccountPermissions is IAccountPermissions, EIP712 {
    ...
    modifier onlyAdmin() virtual {
        require(
            isAdmin(msg.sender),
            "AccountPermissions: caller is not an admin"
        );
        _;
    }
    ...
    function setPermissionsForSigner(
        ...
        require(
            !isAdmin(targetSigner),
            "AccountPermissions: signer is already an admin"
        );
        ...
        (bool success, address signer) = verifySignerPermissionRequest(
            _req,
            _signature
        );
        ...
        address[] memory currentTargets = data
    ...
    function isActiveSigner(address signer) public view returns (bool) {
        AccountPermissionsStorage.Data storage data = AccountPermissionsStorage
            .accountPermissionsStorage();
        SignerPermissionsStatic memory permissions = data.signerPermissions[
            signer
        ];
    ...
    function getAllSigners()
        external
        view
        returns (SignerPermissions[] memory signers)
    {
        AccountPermissionsStorage.Data storage data = AccountPermissionsStorage
            .accountPermissionsStorage();
