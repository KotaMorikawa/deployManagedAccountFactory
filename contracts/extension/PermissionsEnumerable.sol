...
library PermissionsEnumerableStorage {
    bytes32 public constant PERMISSIONS_ENUMERABLE_STORAGE_POSITION =
        keccak256("permissions.enumerable.storage");
...
contract PermissionsEnumerable is IPermissionsEnumerable, Permissions {
    ...
    function getRoleMember(
        ...
        PermissionsEnumerableStorage.Data
            storage data = PermissionsEnumerableStorage
        ...
        for (uint256 i = 0; i < currentIndex; i += 1) {
            if (data.roleMembers[role].members[i] != address(0)) {
                ...
                i == data.roleMembers[role].indexOf[address(0)]
            ) {
                check += 1;
            }
        }
